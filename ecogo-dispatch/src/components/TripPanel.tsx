 ...
 */
export function TripPanel({ trip, onClose }: { trip: TripInfo; onClose: () => void }) {
    ...
    >
      ...
      <div className="scroll trip-body">
        <div className="trip-route">
          <strong>{trip.trip.origin ?? '—'}</strong>
          <span className="arrow">→</span>
          <strong>{trip.trip.dest ?? '—'}</strong>
        </div>
        ...
        <dl className="kv">
          ...
          <dt>Đón khách</dt>
          <dd>
            {seg.pickupAddress ?? seg.pickupLabel ?? '—'} · {time(seg.pickupEta)}
          </dd>

          <dt>Trả khách</dt>
          <dd>
            {seg.dropoffAddress ?? seg.dropoffLabel ?? '—'} · {time(seg.dropoffEta)}
          </dd>
        ...
        {seg.companions.length > 0 && (
          <>
            <div className="lane-head">Khách đi cùng <span className="count">{seg.companions.length}</span></div>
